...
def worker_scan_stock_logic(ticker, stock_name):
    ...
    try:
        ...
        if stats['count'] > 0 and stats['win_rate'] >= 70.0 and stats['avg_profit'] > 0:
            # 💡 在股票確認符合進場條件後，才去抓取三大法人資料（節省資源且不拖慢整體速度）
            ...
            return {
                ...
                "三大法人5日各別買賣超": inst_indiv,  # 👈 新增這行
                "三大法人5日買賣超": inst_total,  # 👈 新增這行
                ...
                if total_rows > 0:
                    ...
                    for i, row in target_df.iterrows():
                        ...
                        try:
                            ...
                            hist = yf.Ticker(ticker_yf).history(period='5d', interval='60m')
                            if not hist.empty and en_cond in hist.columns and ex_cond in hist.columns:
                                if hist[en_cond].iloc[-1]:
                                    status_list.append("🟢 已滿足進場條件")
                                elif hist[ex_cond].iloc[-1]:
